<template>
  <v-main id="lobbyBack">
    ...
    <DialogSection v-bind:cre8_game=cre8_game v-bind:game_name=game_name @close_dialog="close_dialog" @create_game="create_game" />
  </v-main>
</template>
<script>
...
export default {
  ...
  methods : {
     ...
     *
     * 1. 신규 게임을 생성한다.
     * 2. owner 사용자를 게임 플레이어 중 1명으로 추가한다.
     */
    create_game: async function() {
      ...
      if (response.status == 201) {
        // 2. owner 사용자를 게임 플레이어로 추가한다.
        this.join_game(response.data);
      } else if (response.status == 208) {
        this.$dialog.alert_dialog("게임 만들기 실패", "일시적인 에러가 발생했습니다. 에러가 계속되는 경우, 관리자에게 문의해 주세요.");
      } else {
        this.$dialog.alert_dialog("시스템 에러", "예기치 못한 에러가 발생했습니다.");
      }  
    },
      ...
      if (response.data.length == 0) {
        ...
        if (response.data.length < this.max_gamers[game.gamename]) {
          ...
          if (response.status == 201 || response.status == 208) {
            ...
            this.$dialog.alert_dialog("게임 참가 실패", "더이상 참가할 수 없습니다.(최대 " + this.max_gamers[this.game_name] + "명)");
          } else {
            this.$dialog.alert_dialog("시스템 에러", "예기치 못한 에러가 발생했습니다.");
          }  
        } else {
